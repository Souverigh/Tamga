...
// не только текущей вкладкой. 200 всё равно с большим запасом выше того,
// что способен создать один браузер при MAX_CONCURRENT_REQUESTS=10 (упрётся
// в реальную латентность Gemini куда раньше, чем в этот бюджет) — так что
// на практике рероста скорости с 14 узкое место снимается полностью, но
// потолок 1000 остаётся далёким даже при нескольких пачках параллельно.
