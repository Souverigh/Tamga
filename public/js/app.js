...
async function recognizePage(pageImage, mode, lang, presetType, signal, onStatus) {
  if (mode === 'gemini') {
    ...
    // результат" на повёрнутом свидетельстве о рождении). Пробовали чинить
    // это ЧЕРЕЗ GEMINI (сначала — назвать угол числом, затем — сравнить 4
    // готовых варианта) — оба раза недостаточно надёжно на практике:
    // Gemini после сравнения вариантов всё равно не восстановил
    // распознавание на реальном документе, а после РУЧНОГО поворота в
    // правильную сторону — распозналось верно. Теперь поворот определяет
    // Tesseract OSD (см. ocr/orientation.js) — классический алгоритм, а не
    // LLM-угадайка, работает локально в браузере, без лимита страниц и без
