...
// passportCanadaDocx.mjs — файлы вёрстки типов документов друг у друга
// общие константы не импортируют (см. комментарий в тех файлах).
...
export function buildBirthCertificateDocumentXml(translation, certification) {
  ...
  const employeeXml = map.responsibleEmployee?.value
    ? para(`${map.responsibleEmployee.label}:`) + paraRuns(run(`${sigLabel}  `, { italic: true }) + run(map.responsibleEmployee.value, { bold: true }))
    : '';
