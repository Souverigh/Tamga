...
//  - В образце справа от обоих блоков идёт узкая колонка с номером серии
//    страницы паспорта, повёрнутым на 90° ("ELZ 97532") — это не данные
//    владельца, а служебная печатная метка страницы (номер вкладыша), и
//    у обоих блоков она разная, то есть это не то же самое, что номер
//    паспорта. Не воспроизводится — не несёт переводимой информации, а
//    новое поле только под неё того не стоит.
...
export function buildPassportCanadaDocumentXml(translation, certification) {
  ...
  const bioFields = [map.surname, map.givenName, map.citizenship, map.birthDate, map.birthPlace, map.gender, map.issueDate, map.expiryDate, map.issuingAuthority]
    .map(fieldLine)
    .filter(Boolean)
    .join('');
  const bioRow = row(
