...
// полноценной бесплатной заменой платного пакета. Это ЗАПАСНОЙ вариант, если
// TAMGA_FREE_DAILY_PAGE_LIMIT не задана в Vercel — см. ниже.
const DEFAULT_DAILY_LIMIT = 3;

// Ethan, 9 сен 2026: "пока раскручиваемся, нужно временно 20 страниц в
// день" — читаем из переменной окружения, чтобы менять лимит туда-обратно
// БЕЗ передеплоя (Vercel → Settings → Environment Variables →
// TAMGA_FREE_DAILY_PAGE_LIMIT). Не задана/не число/меньше 1 — используем
// запасной DEFAULT_DAILY_LIMIT выше, а не молча ломаем лимит нулём/NaN.
function currentDailyLimit() {
  const raw = process.env.TAMGA_FREE_DAILY_PAGE_LIMIT;
  const parsed = raw ? parseInt(raw, 10) : NaN;
  return Number.isFinite(parsed) && parsed >= 1 ? parsed : DEFAULT_DAILY_LIMIT;
}
...
module.exports = { consumeAnonymousUsage, extractClientIp, hashIp, currentDailyLimit };
