...
const FIELD_LABEL_TRANSLATIONS = {
  ...
  '№ апостиля': { en: 'Apostille No.', zh: '附加证明书编号', de: 'Apostille-Nr.', tr: 'Apostil No.', uz: 'Apostil raqami', kk: 'Апостиль №', ky: 'Апостиль №', ru: '№ апостиля' }
  ...
  ,'Подписан(а)': { en: 'Signed by', zh: '由签署', de: 'Unterzeichnet von', tr: 'İmzalayan', uz: 'Imzolagan', kk: 'Қол қойған', ky: 'Кол койгон', ru: 'Подписан(а)' }
  ,'Действующий(ая) в качестве': { en: 'Acting in the capacity of', zh: '以……身份行事', de: 'Handelnd in der Eigenschaft als', tr: 'Şu sıfatla hareket eden', uz: 'Quyidagi lavozimda ish yurituvchi', kk: 'Мынадай қызметте әрекет етуші', ky: 'Төмөнкү сапатта иштеген', ru: 'Действующий(ая) в качестве' }
  ,'Скреплён печатью/штампом': { en: 'Bears the seal/stamp of', zh: '盖有印章/印戳', de: 'Versiegelt/gestempelt von', tr: 'Mühür/kaşe ile tasdikli', uz: 'Muhr/shtamp bilan tasdiqlangan', kk: 'Мөр/мөртабан басылған', ky: 'Мөөр/штамп менен бекитилген', ru: 'Скреплён печатью/штампом' }
  ,'Удостоверено в': { en: 'At', zh: '地点', de: 'Ort', tr: 'Yer', uz: 'Joy', kk: 'Орын', ky: 'Жери', ru: 'Удостоверено в' }
  ,'Удостоверено органом': { en: 'By', zh: '由……认证', de: 'Durch', tr: 'Tarafından', uz: 'Tasdiqlagan organ', kk: 'Куәландырған орган', ky: 'Күбөлөндүргөн орган', ru: 'Удостоверено органом' }
  ,'Печать/подпись': { en: 'Seal/stamp or signature', zh: '印章/签名', de: 'Siegel/Stempel oder Unterschrift', tr: 'Mühür/kaşe veya imza', uz: 'Muhr/shtamp yoki imzo', kk: 'Мөр/мөртабан немесе қол', ky: 'Мөөр/штамп же кол', ru: 'Печать/подпись' }
};
...
function transliterateName(value, language) {
...
function normalizeDate(value) {
  ...
  if (match) return `${match[1].padStart(2, '0')}-${match[2].padStart(2, '0')}-${match[3].slice(-2)}`;
  return text;
}
...
const APOSTILLE_COMPATIBILITY_KEYS = {
  ...
  seal_signature: 'additional_notes'
};
...
async function recognizeAndTranslateDocument({ base64, mimeType, apiKey, language, clientApiKey, clientSlug, pageCount = 1 }) {
  ...
  for (const [index, { key, label }] of typeDef.fields.entries()) {
    const raw = extracted.find(field => field && field.label === label) || extracted[index] || {};
    const value = typeof raw.value === 'string' ? raw.value : '';
    const preserve = PRESERVE_LABEL.test(label);
    const name = NAME_LABEL.test(label) || /signatory_name|public_document|seal_signature/i.test(key);
    ...
    fieldsByKey[key] = {
      ...
      translated: date ? normalized : (preserve ? value : (name ? transliterateName(value, language) : '')),
      translationStatus: date || preserve ? 'preserved' : (name ? 'transliterated' : 'translated')
    };
    if (value.trim() && !preserve && !name) segments.push({ id: `field_${index}`, key, text: value });
  }
