...
function buildCombinedInstruction(customTypes) {
  ...
  return `Classify the document into exactly one of these types: ${typeList}. If none match, use "Другое". ` +
...
async function recognizeAndTranslateDocument({ base64, mimeType, apiKey, language, clientApiKey, clientSlug, pageCount = 1 }) {
  ...
  for (const [index, { key, label }] of typeDef.fields.entries()) {
    const raw = extracted.find(field => field && field.label === label) || {};
    ...
    if (value.trim()) segments.push({ id: `field_${index}`, key, text: value });
  }
  ...
  if (segments.length) {
    ...
    for (const current of batches) {
      ...
      for (const seg of translated) {
        ...
        if (source && fieldsByKey[source.key]) fieldsByKey[source.key].translated = seg.text;
      }
    }
  }
