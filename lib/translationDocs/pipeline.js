...
const NAME_LABEL = /ФИО|Фамилия|Имя|Отчество|Подписан|Доверитель|Представитель|Сторона|Владелец|Плательщик|Получатель/i;
...
async function recognizeAndTranslateDocument({ base64, mimeType, apiKey, language, clientApiKey, clientSlug, pageCount = 1 }) {
  ...
  for (const [index, { key, label }] of typeDef.fields.entries()) {
    ...
    fieldsByKey[key] = {
      ...
      translated: preserve ? value : (name ? transliterateName(value, language) : ''),
      translationStatus: preserve ? 'preserved' : (name ? 'transliterated' : 'translated')
    };
