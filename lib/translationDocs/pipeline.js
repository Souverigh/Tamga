...
async function recognizeAndTranslateDocument({ base64, mimeType, apiKey, language, clientApiKey, clientSlug, pageCount = 1 }) {
  ...
  for (const [index, { key, label }] of typeDef.fields.entries()) {
    ...
    const name = NAME_LABEL.test(label) || /signatory_name|public_document|seal|signature/i.test(key);
  ...
  if (docType === 'apostille') {
    ...
    elements = sourceElements.map((element, index) => {
      ...
      const isName = NAME_LABEL.test(label) || /signatory_name|public_document|seal|signature/i.test(key);
      const translated = field?.translated || (isName ? transliterateName(value, language) : value);
      return {
        key,
