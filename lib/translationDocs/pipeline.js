...
const PRESERVE_LABEL = /ПИН|ИНН|номер|сч[её]т|IBAN|VIN|дата|серия|БИК|апостил|MRZ|^Тип$|^Код$|Идентификационный код|Код подписи/i;
...
async function recognizeAndTranslateDocument({ base64, mimeType, sourceText, apiKey, language, clientApiKey, clientSlug, pageCount = 1 }) {
  ...
  for (const [index, { key, label, kind }] of typeDef.fields.entries()) {
    ...
    const preserve = !markerKind && (PRESERVE_LABEL.test(label) || /^[\d\s.,:/+()%-]+$/.test(value));
    ...
    if (value.trim() && (docType === 'apostille' ? !fixed : (!preserve && !name && !date && !markerKind))) segments.push({ id: `field_${index}`, key, text: value });
  }
