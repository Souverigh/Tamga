...
async function recognizeAndTranslateDocument({ base64, mimeType, sourceText, apiKey, language, clientApiKey, clientSlug, pageCount = 1 }) {
  ...
  for (const [index, { key, label }] of typeDef.fields.entries()) {
    ...
    fieldsByKey[key] = {
      ...
      translated: fixed?.value ?? (date ? normalized : (preserve ? value : (name ? nameValue : ''))),
      translationStatus: fixed?.status ?? (date ? (normalized === value ? 'preserved' : 'translated') : (preserve ? 'preserved' : (name ? (nameValue === value ? 'preserved' : 'transliterated') : 'translated'))),
