...
async function recognizeAndTranslateDocument({ base64, mimeType, apiKey, language, clientApiKey, clientSlug, pageCount = 1 }) {
  ...
  const { validateApostille, apostilleSignature } = await import('../../public/js/translation/apostille.mjs');
  ...
  for (const [index, { key, label }] of typeDef.fields.entries()) {
    ...
    const preserve = docType === 'apostille' ? ['signatory_name', 'signature', 'certified_date', 'apostille_number', 'public_document', 'certified'].includes(key) : PRESERVE_LABEL.test(label);
    const name = docType !== 'apostille' && (NAME_LABEL.test(label) || /signatory_name|public_document|seal|signature/i.test(key));
    const date = docType !== 'apostille' && isDateField(key, label);
  ...
  if (elements) {
    elements.forEach(e => {
      ...
      if (field) {
        if (e.key === 'signature') field.translated = apostilleSignature(field.value, language);
