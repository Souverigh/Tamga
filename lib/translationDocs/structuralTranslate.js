...
async function translateDocumentSegments({ segments: rawSegments, language, clientApiKey, clientSlug, pageCount = 1 }) {
  ...
  // загруженный .docx = одна страница, независимо от числа абзацев внутри —
  // то же соглашение, что и у остального модуля (pageCount от файла, не от
  // объёма текста).
  ...
  for (const current of batches) {
    const request = validateTranslationRequest({ language, segments: current });
