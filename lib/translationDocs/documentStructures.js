...
const FIELD_DEFINITIONS = {
  ...
  // Поле "Информации о составе семьи" (портал "Тундук", Ethan, 19 сен 2026,
  // реальный пример) — число членов семьи над таблицей самих членов
  // (familyMembers, см. pipeline.js). Значение — цифра, попадает под
  // /^[\d\s.,:/+()%-]+$/ в pipeline.js и остаётся как есть.
  ,memberCount: { label: 'Количество членов семьи' }
...
const STANDARD_TRANSLATION_FIELDS = {
  ...
  // Ещё одна распечатка с портала "Тундук" (Ethan, 19 сен 2026, реальный
  // пример) — набор реквизитов почти как у "Справки о несудимости", но вместо
  // результата услуги есть количество членов семьи и таблица самих членов
  // семьи (не в fields, а в отдельном массиве familyMembers — см. pipeline.js),
  // см. familyCompositionDocx.mjs.
  'Информация о составе семьи': ['country', 'fullName', 'taxId', 'memberCount', 'address', 'issuingAuthority', 'contactNote', 'documentNumber', 'formationDateTime', 'sourceSystemNote', 'legalValidityNote', 'actualityNote', 'formedByAuthority', 'qrCode', 'stampText', 'signatureDate', 'signatureCode'],
...
const TYPE_REGISTRY = Object.fromEntries([
  ...
  ['Справка о несудимости', {
    ...
    hint: 'an official certificate generated by the Kyrgyz Republic\'s "Тундук" (Tunduk) state electronic services portal, confirming whether a named person has a criminal record ("судимость") in the Kyrgyz Republic — headed with the Kyrgyz Republic state coat of arms, referencing the Ministry of Internal Affairs (МВД), and containing the applicant\'s full name and PIN, a "Результат"/"SERVICE RESULT" statement of the finding, an identification code, a document number, a QR code for verification, and an electronic-signature block (signature date and code, e-signature notice). Distinct from the generic "Справка" (an ordinary reference/confirmation letter for employment, income or residency with no e-government portal branding, QR code or electronic-signature block) and from civil registry certificates like "Свидетельство о рождении"/"Свидетельство о смерти", and from the other Tunduk printout "Информация о составе семьи" (which lists family members in a table and has no criminal-record result).',
    preservesParagraphs: false
  }],
  // Тот же приём и та же причина, что у "Справки о несудимости" выше (вне
  // DOC_TYPES, отдельная запись). Главный риск — путаница именно с ней: обе
  // печатаются порталом "Тундук" с QR-кодом и блоком электронной подписи, и
  // различаются только содержимым — поэтому hint прямо называет отличия.
  // familyMembers: true включает в pipeline.js извлечение таблицы членов
  // семьи (schema-поле familyMembers) только для этого типа. Hint снят с
  // ОДНОГО реального образца (русскоязычная распечатка), не с нескольких —
  // поправить, если классификация будет путать типы на других образцах.
  ['Информация о составе семьи', {
    label: 'Информация о составе семьи',
    fields: genericFields(STANDARD_TRANSLATION_FIELDS['Информация о составе семьи']),
    hint: 'an official printout generated by the Kyrgyz Republic\'s "Тундук" (Tunduk) state electronic services portal titled "Информация о составе семьи" (information about family composition), listing the applicant\'s full name, PIN, the number of family members ("Количество членов семьи"), the registered address, and a TABLE of family members with the columns №, Ф.И.О., Степень родства (relationship, e.g. "Сын", "Мать") and Дата рождения — followed by the issuing authority, a document number, the date/time the document was generated, portal notes about data source/legal validity, a QR code, and an electronic-signature block. Distinct from "Справка о несудимости" (the other Tunduk printout: it has a "Результат"/service-result statement about criminal record and NO table of family members) and from the generic "Справка".',
    preservesParagraphs: false,
    familyMembers: true
  }],
