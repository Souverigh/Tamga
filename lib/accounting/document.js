...
// хендовера), act_date (акт, 15 сен 2026) и payment_order_date (платёжное
// поручение, 15 сен 2026) — все ISO-даты, просто разные ключи по типу
// документа.
const DATE_KEYS = new Set(['invoice_date', 'delivery_note_date', 'act_date', 'payment_order_date']);
...
// Платёжное поручение (15 сен 2026) — та же сборка header/normalized, что у
// остальных типов, но НЕ хранит items как содержательные данные (документ не
// имеет таблицы строк) — items здесь всегда прогоняется через ту же логику
// исключительно для единообразия формы doc {header, items, normalized}, но
// rules/payment_order.js его не читает.
function buildPaymentOrderDoc(rawExtraction) {
  const header = rawExtraction.header || {};
  const items = Array.isArray(rawExtraction.items) ? rawExtraction.items : [];
  return {
    header,
...
module.exports = { buildEsfDoc, buildNakladnayaDoc, buildActDoc, buildPaymentOrderDoc };
