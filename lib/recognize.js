...
const { buildExtractionInstruction, extractionSchemaField } = require('./extraction');
...
function buildInstructionAndSchema(knownDocType) {
  ...
  const requiredFields = ['text', 'fields', ...(skipClassification ? [] : ['documentType'])];

  return { instruction: instructionParts.join(' '), schemaProperties, requiredFields, skipClassification };
}
...
async function recognizeDocument({ base64, mimeType, docType }) {
  ...
  const { instruction, schemaProperties, requiredFields, skipClassification } = buildInstructionAndSchema(docType);
  ...
  if (parsed.__unparsed !== undefined) {
    return { text: parsed.__unparsed.trim(), documentType: docType || 'Другое', fields: [] };
  }
