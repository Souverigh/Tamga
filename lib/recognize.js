...
function buildInstructionAndSchema(knownDocType, { skipOcr = false } = {}) {
...
async function recognizeDocument({ base64, mimeType, docType, skipOcr }) {
  validateInput({ base64, mimeType });

  const apiKey = process.env.GEMINI_API_KEY;
  const { instruction, schemaProperties, requiredFields, skipClassification, tableMode } = buildInstructionAndSchema(docType, { skipOcr });
