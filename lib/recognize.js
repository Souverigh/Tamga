...
function buildInstructionAndSchema(knownDocType, { skipOcr = false, includeText = true, customFields = null, customDocTypes = null, formatting = null, fieldOverrides = null } = {}) {
  ...
  return { instruction: instructionParts.join(' '), schemaProperties, requiredFields, skipClassification, tableMode, tableColumns, omitText };
}
...
async function recognizeDocument({ base64, mimeType, docType, skipOcr, includeText, clientApiKey, clientSlug, clientIp, batchId }) {
  ...
  const { instruction, schemaProperties, requiredFields, skipClassification, tableMode, tableColumns, omitText } = buildInstructionAndSchema(docType, { skipOcr, includeText, customFields, customDocTypes, formatting, fieldOverrides });
