...
const { buildOrientationInstruction, orientationSchemaField, normalizeRotation } = require('./orientation');
...
async function detectOrientation({ base64, mimeType, clientIp }) {
  validateInput({ base64, mimeType });
  if (clientIp) {
    const rateLimit = await checkAnonymousRecognizeRateLimit({ ip: clientIp });
  ...
  try {
    const { result } = await callGemini({
      ...
      requiredFields: ['rotation']
    });
    return { rotation: normalizeRotation(result?.rotation) };
  } catch (err) {
    if (err instanceof GeminiError) throw new RecognizeError(err.message, err.status);
    throw err;
  }
