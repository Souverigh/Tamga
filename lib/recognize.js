...
const { ROTATIONS, buildOrientationInstruction, orientationSchemaField, rotationForUprightImage } = require('./orientation');
...
const { checkAnonymousRecognizeRateLimit } = require('./authRateLimit');
...
const { checkForgerySignals } = require('./postprocess/forgerySignals');

const ALLOWED_MIME_TYPES = ['image/png', 'image/jpeg', 'image/webp', 'application/pdf'];
// Лимит приложения для одного файла. Фактический лимит HTTP-платформы должен
// быть не меньше этого значения (Vercel Hobby ограничивает тело примерно 4.5MB).
...
function validateInput({ base64, mimeType }) {
  ...
  if (base64.length > Math.ceil(MAX_INPUT_BYTES / 3) * 4) {
    throw new RecognizeError('Файл слишком большой для одного запроса (лимит 3MiB на страницу/изображение)', 413);
  }
  if (!/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(base64)) {
    throw new RecognizeError('Некорректный base64');
  }
  const bytes = Buffer.from(base64, 'base64');
  if (!bytes.length || bytes.toString('base64') !== base64) throw new RecognizeError('Некорректный base64');
  const hex = bytes.subarray(0, 12).toString('hex');
  const valid = mimeType === 'image/png' ? hex.startsWith('89504e470d0a1a0a') && bytes.length >= 33
    : mimeType === 'image/jpeg' ? hex.startsWith('ffd8ff') && bytes.length >= 4
    : mimeType === 'image/webp' ? bytes.toString('ascii', 0, 4) === 'RIFF' && bytes.toString('ascii', 8, 12) === 'WEBP' && bytes.length >= 20
    : bytes.toString('ascii', 0, 5) === '%PDF-' && /%%EOF\s*$/.test(bytes.subarray(-1024).toString('ascii'));
  if (!valid) throw new RecognizeError('Содержимое файла не соответствует mimeType');
}
...
async function safeRecordUsageEvent({ clientRef, docType, success, confidence, usage, latencyMs }) {
  ...
  try {
    ...
    if (!outcome.ok) {
      console.error('recognize: analytics event rejected');
    }
  } catch (err) {
    console.error('recognize: analytics unavailable');
  }
}

// Библиотека шаблонов документов (lib/documentTemplates.js) — Фаза 1, только
// накопление сигнатур, промпт не меняет (см. комментарий в самом модуле и
// TECH_DEBT.md). Та же область (clientRef), что и safeRecordUsageEvent выше.
// no_fields — ожидаемый и частый исход (табличные типы без totals, __unparsed
// ветка вообще сюда не доходит), не логируется как ошибка.
async function safeRecordDocumentTemplate({ clientRef, docType, fields, confidence }) {
  try {
    const outcome = await recordDocumentTemplate({ clientRef, docType, fields, confidence });
    if (!outcome.ok && outcome.reason !== 'no_fields' && outcome.reason !== 'not_configured') {
      console.error('recognize: document template not recorded —', outcome.reason);
    }
  } catch (err) {
    console.error('recognize: document template unavailable —', err.message);
  }
}
...
async function recognizeDocument({ base64, mimeType, docType, includeText, clientApiKey, clientSlug, clientIp, batchId }) {
  ...
  if (clientApiKey || clientSlug) {
    ...
    if (usage.unavailable) throw new RecognizeError('Учёт лимитов временно недоступен', 503, 'QUOTA_UNAVAILABLE');
    if (!usage.allowed) {
    ...
    // Троттлинг ТОЛЬКО для бесплатного анонимного тарифа (Ethan, 15 сен
    // 2026, "сделай только для бесплатной версии") — проверяем ДО дневного
    // лимита, чтобы заблокированный всплеском запрос не тратил дневную
    // страницу впустую (см. lib/authRateLimit.js:checkAnonymousRecognizeRateLimit).
    const rateLimit = await checkAnonymousRecognizeRateLimit({ ip: clientIp });
    if (rateLimit.unavailable) throw new RecognizeError('Проверка частоты запросов временно недоступна. Запрос в Gemini не отправлен.', 503, 'QUOTA_UNAVAILABLE');
    if (!rateLimit.allowed) {
      throw new RecognizeError(
        `Слишком много запросов подряд с вашего адреса. Попробуйте снова через ${rateLimit.retryAfterSeconds || 60} сек.`,
        429
      );
    }
  ...
  try {
    ...
    // Табличные типы (накладная и т.п.) — по умолчанию НЕ просим у Gemini
    // полную OCR-транскрипцию текста в дополнение к items (15 сен 2026,
    // Ethan: "именно когда готовится накладные там с огромным количеством
    // позиций, там и в основном задержка идёт"). Без этого модель пишет
    // содержимое таблицы ДВАЖДЫ в одном ответе — один раз как text (см.
    // lib/ocr.js), один раз как структурированные items — и для накладной
    // с большим числом строк это ощутимо увеличивает и объём ответа, и
    // время генерации (см. также maxOutputTokens в geminiClient.js).
    // Приоритет (первый определённый побеждает): 1) includeText, явно
    // переданный вызывающим кодом — как раньше, без изменений; 2)
    // formatting.includeText клиента, если задан явно, — клиент может
    // включить текст обратно даже для табличных типов; 3) дефолт по типу:
    // true для карточных (без изменений), false для табличных (было true).
    // "Скачать текст" (public/js/export/txtExport.js) для накладных без
    // явного formatting.includeText=true теперь приходит пустым для этого
    // типа — согласовано с Итаном 15 сен 2026, структурные экспорты
    // (CSV/XLSX/JSON) это не затрагивает, они строятся из items/fields.
    if (includeText === undefined) {
      includeText = formatting?.includeText !== undefined ? formatting.includeText : !isTableType(docType);
    }
  ...
  await safeRecordDocumentTemplate({ clientRef: usageClientRef, docType: result.documentType, fields: result.fields, confidence: result.confidence });
  ...
  // Признаки подделки/редактирования (lib/postprocess/forgerySignals.js) —
  // отдельные от бизнес-правил, level:'suspicious', см. комментарий в самом
  // модуле. Всегда добавляются (даже анонимным посетителям — сознательно:
  // этот сигнал не про клиентскую настройку, а про сам файл).
  const forgeryWarnings = await checkForgerySignals({ base64, mimeType, fields: result.fields });
  if (forgeryWarnings.length) result.warnings = [...result.warnings, ...forgeryWarnings];

  return result;
}

// Дешёвая предварительная проверка ориентации страницы — НЕ списывает лимит
// страниц (в отличие от recognizeDocument выше) и не трогает конфиг клиента:
// это не распознавание документа, а лишь угол поворота, чтобы вызывающий
// (клиент, см. public/js/ocr/orientation.js) мог сам повернуть картинку перед
// единственным платным запросом. Анонимные вызовы всё равно защищены общим
// троттлингом по IP (тем же, что и у обычного распознавания), а суммарные
// расходы Gemini — общим бюджетом (см. reserveGeminiBudget внутри callGemini).
//
// images — РОВНО 4 варианта одной и той же уменьшенной страницы, повёрнутые
// клиентом на 0/90/180/270° (см. lib/orientation.js — почему сравнение
// готовых вариантов, а не просьба назвать угол числом).
async function detectOrientation({ images, clientIp }) {
  if (!Array.isArray(images) || images.length !== ROTATIONS.length) {
    throw new RecognizeError(`Поле "images" должно содержать ровно ${ROTATIONS.length} варианта`, 400);
  }
  images.forEach(img => validateInput({ base64: img?.base64, mimeType: img?.mimeType }));
  if (clientIp) {
    const rateLimit = await checkAnonymousRecognizeRateLimit({ ip: clientIp });
    if (rateLimit.unavailable) throw new RecognizeError('Проверка частоты запросов временно недоступна.', 503, 'QUOTA_UNAVAILABLE');
    if (!rateLimit.allowed) {
      throw new RecognizeError(`Слишком много запросов подряд с вашего адреса. Попробуйте снова через ${rateLimit.retryAfterSeconds || 60} сек.`, 429);
    }
  }
  const apiKey = process.env.GEMINI_API_KEY;
  try {
    const { result } = await callGemini({
      apiKey,
      instruction: buildOrientationInstruction(),
      images: images.map((img, i) => ({ mimeType: img.mimeType, base64: img.base64, label: `Image ${i + 1}:` })),
      schemaProperties: orientationSchemaField(),
      requiredFields: ['uprightImage']
    });
    return { rotation: rotationForUprightImage(result?.uprightImage) };
  } catch (err) {
    if (err instanceof GeminiError) throw new RecognizeError(err.message, err.status);
    throw err;
  }
...
module.exports = { recognizeDocument, detectOrientation, RecognizeError, ALLOWED_MIME_TYPES };
