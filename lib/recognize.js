...
function buildInstructionAndSchema(knownDocType, { skipOcr = false, customFields = null, customDocTypes = null, formatting = null } = {}) {
  ...
  instructionParts.push(buildExtractionInstruction(knownDocType, { customFields, customDocTypes, formatting }));
...
async function recognizeDocument({ base64, mimeType, docType, skipOcr, clientApiKey, clientSlug }) {
  ...
  const { instruction, schemaProperties, requiredFields, skipClassification, tableMode } = buildInstructionAndSchema(docType, { skipOcr, customFields, customDocTypes, formatting });
  ...
  return {
    ...
    items: normalizeItems(rawItems, finalDocType, columnsForType, keysForType, formatting)
  };
}
