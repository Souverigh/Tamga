...
const { isTableType, columnsForType, keysForType } = require('./docSchema');
const { normalizeFields, normalizeItems } = require('./fieldFormat');
const { callGemini, GeminiError } = require('./geminiClient');
const { getCustomFields } = require('./customFieldsLookup');
...
function buildInstructionAndSchema(knownDocType, { skipOcr = false, customFields = null } = {}) {
  ...
  instructionParts.push(buildExtractionInstruction(knownDocType, { customFields }));

  const schemaProperties = {
    ...(skipOcr ? {} : ocrSchemaField()),
    ...(skipClassification ? {} : classificationSchemaField()),
    ...(tableMode ? itemsSchemaField(knownDocType) : extractionSchemaField())
  };
...
// (см. api/v1/recognize.js), используется только чтобы найти для него кастомный
// список полей в Supabase (см. customFieldsLookup.js). Веб-интерфейс его никогда
// не передаёт — поведение по умолчанию не меняется ни для кого, кроме интеграций
// с настроенной кастомизацией под их ключ.
async function recognizeDocument({ base64, mimeType, docType, skipOcr, clientApiKey }) {
  validateInput({ base64, mimeType });

  const apiKey = process.env.GEMINI_API_KEY;
  const customFields = clientApiKey ? await getCustomFields(clientApiKey) : null;
  const { instruction, schemaProperties, requiredFields, skipClassification, tableMode } = buildInstructionAndSchema(docType, { skipOcr, customFields });
  ...
  return {
    ...
    // Нормализация форматов (даты → ДД.ММ.ГГГГ, суммы → запятая) — единое место
    // для веб-интерфейса и для публичного API (api/v1/recognize.js), которым оба
    // пользуются этой же функцией recognizeDocument.
    fields: normalizeFields(rawFields),
    items: normalizeItems(rawItems, finalDocType, columnsForType, keysForType)
  };
}
