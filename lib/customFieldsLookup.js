...
async function getClientConfig({ apiKey, clientSlug } = {}) {
  ...
  // делали их под свои нужды") — тот же приём, что maxConcurrency выше:
  // живут внутри formatting (JSONB), отдельная колонка не заводилась.
  // Единственный поддерживаемый тип в первой версии — percentage_match (см.
  // public/js/postprocess/businessRules.js). Не доверяем содержимому из БД
  // вслепую (админка не валидирует ввод строго, а строку можно поправить и
  // напрямую в Supabase) — отфильтровываем записи без обязательных полей и
  // зажимаем tolerancePercent в разумный диапазон, а не передаём как есть.
  const rawBusinessRules = Array.isArray(formatting && formatting.businessRules) ? formatting.businessRules : [];
  const businessRules = rawBusinessRules
    .filter(r => r && r.type === 'percentage_match' && typeof r.baseField === 'string' && r.baseField.trim()
      && typeof r.valueField === 'string' && r.valueField.trim() && Number.isFinite(Number(r.expectedPercent)))
    .map(r => ({
      type: 'percentage_match',
      baseField: r.baseField.trim(),
      valueField: r.valueField.trim(),
      expectedPercent: Number(r.expectedPercent),
      tolerancePercent: Number.isFinite(Number(r.tolerancePercent)) ? Math.max(0, Math.min(50, Number(r.tolerancePercent))) : 1,
      level: r.level === 'info' ? 'info' : 'error'
    }));
