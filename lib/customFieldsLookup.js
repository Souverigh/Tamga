...
const SELECT_COLUMNS = 'fields,field_overrides,custom_doc_types,formatting,display_name,logo_url,accent_color';
...
async function getClientConfig({ apiKey, clientSlug } = {}) {
  ...
  // ни кастомных типов, ни форматирования, ни фасада) — ведём себя так же,
  // как если бы строки не было вовсе: нет смысла считать "найден, но ничего
  // не задано" особым случаем.
  if (!fields && !fieldOverrides && !customDocTypes && !formatting && !row.display_name && !row.logo_url && !row.accent_color) {
    return null;
  }
