...
const SELECT_COLUMNS = 'fields,field_overrides,custom_doc_types,formatting,display_name,logo_url,accent_color,access_password_hash,page_limit,pages_used';

const { validateBusinessRules } = require('./clientConfigValidation');
...
async function fetchConfigRow(column, value, fresh = false) {
  const cacheKey = `${column}:${value}`;
  const cached = fresh ? undefined : getCached(cacheKey);
  if (cached !== undefined) return cached;

  const supabaseUrl = process.env.SUPABASE_URL;
  const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!supabaseUrl || !serviceKey) { if (fresh) throw new Error('Client configuration unavailable'); return null; } // кастомизация просто не настроена — это нормально, не кэшируем
  ...
  try {
    ...
    if (!res.ok) {
      if (fresh) throw new Error('Client configuration unavailable');
      console.error('customFieldsLookup: lookup failed');
      return null;
    }
...
//
// Accounting is mandatory: errors deny processing with unavailable:true.
async function consumeUsage({ apiKey, clientSlug }) {
  const supabaseUrl = process.env.SUPABASE_URL;
  const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!supabaseUrl || !serviceKey) return { allowed: false, unavailable: true, pagesUsed: 0, pageLimit: null };
  ...
  try {
    ...
    if (!res.ok) {
      console.error('consumeUsage: Supabase RPC вернул', res.status, '— учёт недоступен');
      return { allowed: false, unavailable: true, pagesUsed: 0, pageLimit: null };
    }
    const rows = await res.json();
    const row = Array.isArray(rows) ? rows[0] : rows;
    if (typeof row?.allowed !== 'boolean') throw new Error('Invalid quota response');
    return { allowed: row.allowed, pagesUsed: row.pages_used, pageLimit: row.page_limit };
  } catch (err) {
    console.error('consumeUsage: ошибка запроса к Supabase, учёт недоступен:', err.message);
    return { allowed: false, unavailable: true, pagesUsed: 0, pageLimit: null };
  }
}
...
async function getClientConfig({ apiKey, clientSlug, fresh = false } = {}) {
  const row = apiKey
    ? await fetchConfigRow('api_key', apiKey, fresh)
    : clientSlug
      ? await fetchConfigRow('client_slug', clientSlug, fresh)
      : null;
  if (!row) return null;

  const pageLimit = row.page_limit == null ? null : Number(row.page_limit);
  const pagesUsed = Number(row.pages_used) || 0;
  ...
  // Приоритетная обработка (Ethan, 7 сен 2026) — премиум-клиент может получить
  // персональный потолок одновременных запросов выше дефолтного (см.
  // MAX_CONCURRENT_REQUESTS в app.js), настраивается вручную через /admin как
  // formatting.maxConcurrency (JSON) — отдельная колонка в Supabase ради ЭТОГО
  // не заводилась, formatting и так уже произвольный JSONB под клиентские
  // настройки, лишняя миграция не нужна. Зажимаем диапазон СЕРВЕРНО (не
  // доверяем значению из БД вслепую — админка не валидирует ввод строго):
  // верхняя граница 60 — совместно с app.js:GEMINI_RPM_BUDGET-масштабированием
  // это даёт максимум ~480 RPM персонального бюджета, что всё ещё оставляет
  // запас от реального потолка проекта в 1000 RPM (см. GEMINI_RPM_BUDGET) для
  // остальных каналов/клиентов, работающих с тем же ключом одновременно.
  const rawMaxConcurrency = formatting && Number(formatting.maxConcurrency);
  const maxConcurrency = Number.isFinite(rawMaxConcurrency) ? Math.max(1, Math.min(60, Math.round(rawMaxConcurrency))) : null;

  // Настраиваемые бизнес-правила (Ethan, 7 сен 2026, "чтобы сами компании
  // делали их под свои нужды", расширено 8 сен 2026 с одного типа
  // (percentage_match) до пяти) — тот же приём, что maxConcurrency выше:
  // живут внутри formatting (JSONB), отдельная колонка не заводилась.
  // Переиспользуем ТОТ ЖЕ валидатор, что и на запись (validateBusinessRules,
  // lib/clientConfigValidation.js) — не доверяем содержимому из БД вслепую
  // (строку можно поправить и напрямую в Supabase), а невалидные записи
  // отбрасываем целиком (ошибку показать здесь уже некому — это чтение для
  // самого распознавания, не форма — поэтому просто пропускаем плохие
  // записи, а не роняем всё чтение конфига целиком).
  const rawBusinessRules = Array.isArray(formatting && formatting.businessRules) ? formatting.businessRules : [];
  const businessRules = rawBusinessRules.reduce((acc, rule) => {
    const { value } = validateBusinessRules([rule]);
    if (value && value.length) acc.push(value[0]);
    return acc;
  }, []);

  // Наличие записи определяет клиентский пакет, даже без кастомного фасада.
  ...
  return {
    ...
    // Число или null — см. комментарий выше. В отличие от formatting целиком,
    // это единственное значение безопасно отдавать в браузер как есть (не
    // содержит подсказок промпта) — см. api/client-config.js.
    maxConcurrency,
    // Настраиваемые бизнес-правила (см. выше) — тоже безопасно отдавать в
    // браузер целиком: описывают проверку УЖЕ извлечённых и показанных
    // пользователю значений (public/js/postprocess/businessRules.js), не
    // содержат ничего из промпта Gemini — в отличие от fields/fieldOverrides/
    // customDocTypes, которые НИКОГДА не уходят в ответ браузеру.
    businessRules,
