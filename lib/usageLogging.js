...
// tableFollowUp — сработал ли внутренний дозапрос за строками таблицы (см.
// lib/recognize.js) — Ethan, 9 сен 2026: раньше здесь было поле skipOcr,
// отражавшее, что ЭТО отдельный лог-вызов служебного дозапроса; после
// переноса дозапроса внутрь recognizeDocument (закрытие дыры с
// подсчётом страниц) лог-вызов на страницу теперь всегда ровно один,
// а этот флаг просто говорит, потребовался ли для неё дозапрос технически.
function logRecognition({ docType, confidence, tableMode, tableFollowUp, usage, latencyMs, clientSlug, clientApiKey, clientIp }) {
  try {
