...
// Резолвит токен в личность: {username, role, translatorName, certification}
// — или null, если токен не проходит проверку (истёк, подделан, ссылается на
// удалённого пользователя/сменённый пароль). username==='' в токене —
// легаси-путь без отдельных пользователей, personhood — сам клиент, роль
// всегда 'owner' (ровно старое поведение, ничего не меняется для клиентов
// без пользователей). certification — личный вариант приписки переводчика
// (Ethan, 21 сен 2026, см. api/client-profile.js) — null у легаси-пути и у
// пользователей, которые свой вариант не задавали (используется общий
// приписки клиента).
async function resolveIdentity({ clientSlug, token, fallbackPasswordHash }) {
  ...
  let passwordHash, role, username, translatorName, certification;
  if (decoded.username) {
  ...
  return { username, role, translatorName, certification };
}
