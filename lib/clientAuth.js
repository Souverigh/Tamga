...
function signToken(clientSlug, passwordHash) {
...
function checkClientGate({ clientSlug, passwordHash, token }) {
  ...
  if (!verifyToken(token, clientSlug, passwordHash)) {
    return { ok: false, status: 401, message: 'Требуется пароль доступа' };
  }
  return { ok: true };
}

// Гейт для самообслуживания клиента (api/client-settings.js, Ethan, 8 сен
// 2026: "чтобы клиенты сами меняли поля/типы/бизнес-правила") — СТРОЖЕ, чем
// checkClientGate выше. checkClientGate возвращает ok:true, если пароль вообще
// не задан — это верно для самого ИНСТРУМЕНТА распознавания (нет пароля —
// нет гейта, как обычный сайт без ограничений). Для НАСТРОЕК так нельзя:
// это ЗАПИСЬ в конфигурацию клиента (поля/типы/бизнес-правила/бренд), а не
// чтение — отсутствие пароля не должно означать "открыто для всех", иначе
// любой, кто узнает/угадает slug, смог бы менять чужую конфигурацию. Поэтому
// здесь — всегда требуем валидный токен, независимо от того, задан ли пароль.
function requireClientSettingsAuth({ clientSlug, passwordHash, token }) {
  if (!passwordHash) {
...
module.exports = { hashPassword, verifyPassword, signToken, verifyToken, checkClientGate, requireClientSettingsAuth };
