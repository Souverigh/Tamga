...
const DOC_TYPE_HINTS = {
  ...
  'Аттестат': 'a school certificate of basic or general secondary education, showing the graduate, school, issue year, subjects and grades, final examinations, signatures and certificate number',
  ...
  // "Акт сдачи-приёмки" НАМЕРЕННО убран из алиасов (Ethan, 10 сен 2026,
  // "кыргызский акт приёма-передачи товара путался с актом работ") — эта
  // формулировка слишком близка по смыслу к "акту приёма-передачи" физического
  // товара (см. "Другое" ниже), из-за чего кыргызская формулировка "Кабыл
  // алуу-өткөрүп берүү актысы" (= "акт приёма-сдачи") иногда перетягивала
  // классификацию сюда, хотя уточнение "not physical goods" должно было её
  // отсечь. Для русского/английского/китайского заголовки такого буквального
  // акта приёма-передачи товара ("Акт приема-передачи", "Delivery & Acceptance
  // Record", "交付验收记录") и так не совпадали с этим алиасом — баг проявлялся
  // именно на кыргызском.
  'Акт выполненных работ': 'a document titled "Акт выполненных работ" — a certificate of completed WORK OR SERVICES rendered, with a table of line items for labor/services, not physical goods. NOT a delivery/acceptance act for physical goods (e.g. "Акт приема-передачи", "Кабыл алуу-өткөрүп берүү актысы") — those belong under "Другое" unless a more specific type matches.',
...
const DOC_FIELDS = {
  ...
  'Аттестат': ['Вид аттестата', 'ФИО выпускника', 'Дата рождения', 'Учебное заведение', 'Населённый пункт', 'Год окончания', 'Серия и номер', 'Дата выдачи', 'Регистрационный номер', 'Язык обучения', 'Председатель/директор', 'Заместитель директора', 'Классный руководитель', 'Печать', 'QR-код/ссылка проверки', 'Предметы и оценки', 'Итоговые экзамены и оценки'],
  ...
  // Ставка НДС/Сумма НДС (Ethan, 7 сен 2026, в рамках "совместимость формата
  // экспорта с 1С") — без разбивки по НДС документ нельзя корректно провести
  // в 1С-бухгалтерию, а сам формат файла (чистый Excel/CSV, один заголовок,
  // строка = позиция) уже подходит для стандартного мастера 1С "Загрузка
  // данных из табличного документа" — см. TECH_DEBT.md. Оба поля читаются
  // с документа как есть (на УПД/счёте-фактуре они обычно уже напечатаны
  // отдельными колонками по закону), модель их не вычисляет.
  // totals (Ethan, 9 сен 2026, "НДС стоит в документе, но не распознаётся") —
  // ОТДЕЛЬНЫЙ, документ-уровневый набор label/value полей в дополнение к
  // построчным vatRate/vatSum выше. Причина: на многих реальных счетах-фактурах/
  // накладных/актах НДС указан ТОЛЬКО одним блоком в итогах документа (Сумма без
  // НДС/Ставка/Сумма НДС/Итого), а построчно колонки НДС пустые — предыдущее
  // допущение (см. комментарий выше про "печатаются отдельными колонками по
  // закону") не покрывало этот, более частый на практике, вариант. Оба набора
  // независимы и не исключают друг друга — построчный НДС читается как раньше,
  // totals читается ДОПОЛНИТЕЛЬНО, если модель находит такой блок в документе.
  'Накладная / УПД': {
    mode: 'table',
  ...
  'Техпаспорт автомобиля': ['Владелец', 'Марка / модель', 'Цвет', 'Госномер', 'VIN', 'Год выпуска', 'Серия и номер', 'Дата выдачи'],
...
// если у табличного типа их нет (например, «Справочник номенклатуры»: это
// каталог, не транзакция, никаких сумм/НДС в нём в принципе не бывает).
function totalsForType(docType) {
  const entry = DOC_FIELDS[docType];
