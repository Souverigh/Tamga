...
async function reserveGeminiBudget({ mimeType, instruction, imageCount }) {
  ...
  // imageCount — несколько картинок в одном запросе (см. lib/orientation.js) —
  // без него оценка считала бы место под ОДНУ картинку, даже если реально
  // отправлено четыре.
  const tokens = (mimeType === 'application/pdf' ? 1048576 : 32768) * positive(imageCount, 1) + 8192 + String(instruction).length;
  let row;
  try {
