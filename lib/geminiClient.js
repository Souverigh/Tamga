...
function buildRequestBody({ instruction, mimeType, base64, schemaProperties, requiredFields, sourceText }) {
  return {
...
async function callGemini({ apiKey, instruction, mimeType, base64, schemaProperties, requiredFields, sourceText }) {
  if (!apiKey) {
    throw new GeminiError('GEMINI_API_KEY не настроен на сервере', 500);
  }

  try { await reserveGeminiBudget({ mimeType, instruction: instruction + (sourceText || '') }); } catch (error) { throw new GeminiError(error.message, error.status || 503); }
  const res = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent`,
    {
      ...
      body: JSON.stringify(buildRequestBody({ instruction, mimeType, base64, schemaProperties, requiredFields, sourceText }))
    }
  );
