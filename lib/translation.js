...
const LANGUAGES = {ru:'Russian',ky:'Kyrgyz',en:'English',kk:'Kazakh',uz:'Uzbek',tr:'Turkish',zh:'Chinese',de:'German',it:'Italian',es:'Spanish'};
...
function mergeUsage(a, b) {
  const keys = ['promptTokenCount','candidatesTokenCount','totalTokenCount','thoughtsTokenCount','cachedContentTokenCount'];
  const merged = {};
  for (const k of keys) {
    const av = Number.isFinite(a?.[k]) ? a[k] : null, bv = Number.isFinite(b?.[k]) ? b[k] : null;
    if (av !== null || bv !== null) merged[k] = (av || 0) + (bv || 0);
  }
  return Object.keys(merged).length ? merged : null;
}

// Второй, ПРОВЕРОЧНЫЙ прогон — отдельный вызов Gemini, сравнивающий каждую
// пару источник/перевод ПО СМЫСЛУ (Ethan, 21 сен 2026: "точность падает
// сильно, если проверяется только один раз" — просит второй прогон вместо
// того, чтобы полагаться на единственную проверку). В отличие от
// validateTranslationResponse выше (мгновенная, бесплатная, но видит только
// цифры/форму ответа) — эта проверка ловит любую смысловую порчу: пропущенный
// или добавленный факт, изменённое значение, не только цифры. Дороже (ещё
// один вызов Gemini на КАЖДЫЙ перевод, а не только на подозрительные) и
// медленнее — цена, на которую Ethan сознательно пошёл ради точности.
async function verifyTranslationAccuracy(pairs) {
  const instruction = 'You are a translation QA reviewer, not a translator. You receive JSON pairs {id, source, translated}. For each pair, judge whether "translated" preserves every fact from "source" without adding, dropping, or changing any information (numbers, dates, amounts, names, identifiers, meaning). Formatting differences are NOT errors: a different date order or separator, punctuation, capitalization, or word order alone does not count as a mismatch as long as the same facts are present. Set ok=false ONLY if real content was added, dropped, or changed to a different value or meaning. Respond with exactly one verdict per id; include a brief reason only when ok is false.';
  const { result, usage } = await callGemini({
    apiKey: process.env.GEMINI_API_KEY, instruction, sourceText: JSON.stringify(pairs),
    schemaProperties: { results: { type: 'ARRAY', items: { type: 'OBJECT', properties: { id: { type: 'STRING' }, ok: { type: 'BOOLEAN' }, reason: { type: 'STRING' } }, required: ['id', 'ok'] } } },
    requiredFields: ['results']
  });
  if (!Array.isArray(result?.results)) throw new Error('Проверочный ответ не по схеме');
  return { verdicts: result.results, usage };
}
...
async function translateSegments(request, clientRef) {
  ...
  // Второй прогон (см. verifyTranslationAccuracy выше) — best-effort: если сам
  // проверочный вызов не удался (сеть/бюджет Gemini/невалидный JSON), это НЕ
  // считается подтверждённым расхождением — один сбойный служебный вызов не
  // должен блокировать вообще ЛЮБОЙ перевод. Блокируем только если проверка
  // РЕАЛЬНО прошла и явно указала на несовпадение.
  let verifyUsage = null;
  try {
    const pairs = remaining.map((s, i) => ({id:s.id, source:s.text, translated:geminiSegments[i].text}));
    const verified = await verifyTranslationAccuracy(pairs);
    verifyUsage = verified.usage;
    const vmetric = key => Number.isFinite(verifyUsage?.[key]) ? verifyUsage[key] : null;
    console.info(JSON.stringify({event:'translation.verify.usage',model:GEMINI_MODEL,inputTokens:vmetric('promptTokenCount'),outputTokens:vmetric('candidatesTokenCount'),totalTokens:vmetric('totalTokenCount')}));
    const mismatch = verified.verdicts.find(v => v && v.ok === false);
    if (mismatch) {
      await safeRecordTranslationUsage({clientRef, success:false, usage:mergeUsage(usage,verifyUsage), latencyMs:Date.now()-startedAt});
      throw Object.assign(fail('Перевод изменил номер или сумму либо пропустил фрагмент. Нужна повторная проверка.',502), {confirmedMismatch:true});
    }
  } catch (error) {
    if (error.confirmedMismatch) throw error;
    // Сам проверочный вызов не удался (сеть/бюджет/невалидный JSON) —
    // fail-open: логируем и пропускаем перевод, а не блокируем его тем, что
    // сломалась именно эта вторая, необязательная проверка.
    console.error('translation: проверочный прогон недоступен —', error.message);
  }

  await safeRecordTranslationUsage({clientRef, success:true, usage:mergeUsage(usage,verifyUsage), latencyMs:Date.now()-startedAt});
  for (const seg of geminiSegments) byId.set(seg.id, seg);
  return {segments: request.segments.map(s => byId.get(s.id)), usage:mergeUsage(usage,verifyUsage)};
}
