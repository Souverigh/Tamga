...
function validateBusinessRules(value) {
  ...
  for (const rule of value) {
    ...
    if (rule.docTypes !== undefined) {
      if (!Array.isArray(rule.docTypes) || !rule.docTypes.length || rule.docTypes.some(type => typeof type !== 'string' || !type.trim())) {
        return { error: 'Выберите хотя бы один тип документа или «Все документы»' };
      }
      cleanedRule.docTypes = [...new Set(rule.docTypes.map(type => type.trim()))];
    }
