...
function buildExtractionInstruction(knownDocType) {
  if (knownDocType && isTableType(knownDocType)) {
    ...
    return `The document type is already known: "${knownDocType}" — ${description}. Extract EVERY row as an object with keys ${JSON.stringify(keys)} corresponding to these Russian columns in order: ${JSON.stringify(columns)}. Leave a key as an empty string if that cell is missing for a row. Return one object per row, in the same order as in the document. ${FORMAT_GUIDANCE} ${UNCERTAINTY_GUIDANCE}`;
  }
  if (knownDocType && DOC_TYPES.includes(knownDocType)) {
    const fields = fieldsForType(knownDocType);
    return `The document type is already known: "${knownDocType}". Extract structured fields as label/value pairs using EXACTLY these Russian field labels (leave value as an empty string if not present in the document): ${JSON.stringify(fields)}. ${FORMAT_GUIDANCE} ${UNCERTAINTY_GUIDANCE}`;
  }
  return `Based on the detected category, extract structured fields as label/value pairs. Use EXACTLY the Russian field labels for that category from this mapping (leave value as an empty string if not present in the document): ${JSON.stringify(flatFieldsMap())}. ${FORMAT_GUIDANCE} ${UNCERTAINTY_GUIDANCE}`;
}
