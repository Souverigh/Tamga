...
function buildExtractionInstruction(knownDocType, { customFields, customDocTypes, formatting } = {}) {
  ...
  if (knownDocType && DOC_TYPES.includes(knownDocType)) {
    ...
    // per API-ключ/slug конкретного клиента. Если задан — заменяет стандартный
    // список полей типа целиком: клиенту нужны именно эти N полей, а не всё подряд.
  ...
  // должна покрывать и глобальные типы, и кастомные типы этого клиента, иначе
  // если Gemini классифицирует документ в кастомный тип, извлекать поля будет не по чему.
  const fieldsMap = customDocTypes
    ? { ...flatFieldsMap(), ...Object.fromEntries(Object.entries(customDocTypes).map(([type, entry]) => [type, entry.fields || ['Значение']])) }
    : flatFieldsMap();
