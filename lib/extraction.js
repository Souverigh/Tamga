...
function buildExtractionInstruction(knownDocType, { customFields, customDocTypes, formatting, fieldOverrides } = {}) {
  ...
  if (customDocTypes && customDocTypes[knownDocType]) {
    ...
    return `The document type is already known: "${knownDocType}"${context}. Extract structured fields as label/value pairs using EXACTLY these Russian field labels (leave value as an empty string if not present in the document): ${JSON.stringify(fields)}. ${FORMAT} ${TRANSCRIPTION_GUIDANCE} ${UNCERTAINTY_GUIDANCE} ${FIELD_CONFIDENCE_GUIDANCE}`;
  }
  ...
  if (knownDocType && DOC_TYPES.includes(knownDocType)) {
    ...
    return `The document type is already known: "${knownDocType}". Extract structured fields as label/value pairs using EXACTLY these Russian field labels (leave value as an empty string if not present in the document): ${JSON.stringify(fields)}. ${FORMAT} ${TRANSCRIPTION_GUIDANCE} ${UNCERTAINTY_GUIDANCE} ${FIELD_CONFIDENCE_GUIDANCE}`;
  }
  ...
  const fieldsMap = customDocTypes
    ...
    : withOverrides;
  return `Based on the detected category, extract structured fields as label/value pairs. Use EXACTLY the Russian field labels for that category from this mapping (leave value as an empty string if not present in the document): ${JSON.stringify(fieldsMap)}. ${FORMAT} ${TRANSCRIPTION_GUIDANCE} ${UNCERTAINTY_GUIDANCE} ${FIELD_CONFIDENCE_GUIDANCE}`;
}
