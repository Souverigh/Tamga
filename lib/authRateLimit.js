...
//
// Fail closed: unavailable or malformed accounting blocks the request.
//
...
async function callRpc(fnName, args) {
  ...
  if (!supabaseUrl || !serviceKey) return null; // не настроено — вызывающий код трактует null как отказ
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`authRateLimit: ${fnName} вернул`, res.status, '— проверка недоступна');
      return null;
    }
    const rows = await res.json();
    return Array.isArray(rows) ? rows[0] : rows;
  } catch (err) {
    console.error(`authRateLimit: ошибка запроса ${fnName}, проверка недоступна:`, err.message);
    return null;
  }
}
...
// Supabase/при сбое — allowed:false, unavailable:true.
async function isRateLimited(key, { windowSeconds, maxAttempts }) {
  ...
  if (!row || (typeof row.limited !== 'boolean' && typeof row.allowed !== 'boolean')) return { allowed: false, unavailable: true, retryAfterSeconds: 30 };
  return { allowed: row.limited === false, retryAfterSeconds: row.retry_after_seconds || 0 };
}
...
async function checkFeedbackRateLimit({ ip }) {
  ...
  if (!row || (typeof row.limited !== 'boolean' && typeof row.allowed !== 'boolean')) return { allowed: false, unavailable: true, retryAfterSeconds: 30 }; // fail closed
...
// тарифа сайта (Ethan, 15 сен 2026, в ответ на совет извне про защиту от
// накрутки API-запросов: "сделай только для бесплатной версии"). Платных
// клиентов (api_key/client_slug) это не касается — у них уже есть свой
// лимит страниц (lib/customFieldsLookup.js:consumeUsage) и общий
// RPM/TPM-бюджет на все запросы сразу (lib/geminiBudget.js); душить их ещё
// и коротким IP-окном не нужно, только мешает легитимной пакетной загрузке.
//
// Окно короче и порог выше, чем у feedback/auth (те — про подбор пароля,
// здесь — про то, чтобы не дать скрипту устроить всплеск запросов за
// секунды): достаточно свободно для реальной многостраничной загрузки
// одним анонимным посетителем, но блокирует автоматизированный флуд с
// одного IP. Как и checkFeedbackRateLimit — считаем КАЖДУЮ попытку, не
// только неудачные (consume_recognize_attempt, тот же приём).
const RECOGNIZE_WINDOW_SECONDS = 60;
const RECOGNIZE_MAX_ATTEMPTS = 15;

async function checkAnonymousRecognizeRateLimit({ ip }) {
  const key = `recognize:${hashIp(ip || 'unknown')}`;
  const row = await callRpc('consume_recognize_attempt', { p_key: key, p_window_seconds: RECOGNIZE_WINDOW_SECONDS, p_max_attempts: RECOGNIZE_MAX_ATTEMPTS });
  if (!row || typeof row.allowed !== 'boolean') return { allowed: false, unavailable: true, retryAfterSeconds: 30 }; // fail closed
  return { allowed: !!row.allowed, retryAfterSeconds: row.retry_after_seconds || 0 };
}

module.exports = { checkClientAuthRateLimit, recordClientAuthFailure, checkAdminRateLimit, recordAdminFailure, checkFeedbackRateLimit, checkAnonymousRecognizeRateLimit };
