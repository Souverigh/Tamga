...
// confidence на каждое поле (Ethan, 7 сен 2026, "уверенность по каждому полю" —
// только для карточных типов, см. lib/extraction.js:FIELD_CONFIDENCE_GUIDANCE) —
// нормализуется той же функцией, что и confidence на весь документ (клэмп
// 0-100, null при отсутствии/не-числовом ответе), не доверяем ответу модели
// вслепую. Поля без confidence в исходном объекте (например, из офлайн
// эвристики public/js/extraction/heuristicExtractor.js, которая не проходит
// через этот модуль вообще — тот путь целиком на клиенте) сюда не попадают;
// здесь просто нормализуется то, что реально пришло от Gemini.
function normalizeFields(fields, formatting) {
  if (!Array.isArray(fields)) return fields;
  return fields.map(f => ({
    ...f,
    value: normalizeValueByLabel(f.label, f.value, formatting),
    confidence: normalizeConfidence(f.confidence)
  }));
}

// columns/keys — уже разрешённая раскладка табличного типа (стандартная ЛИБО
// клиентский override, см. lib/extraction.js:resolveTableColumns) — вызывающий
// код (recognize.js) сам решает, какая из них нужна, эта функция больше не
// знает о докTипе и статичной схеме вообще. Роль колонки (дата/сумма) всё
// равно определяется по её РУССКОЙ подписи — работает одинаково для
// стандартных колонок и для клиентских override-названий.
function normalizeItems(items, columns, keys, formatting) {
...
// Уверенность модели (0-100, self-reported, см. lib/confidence.js) — не
// доверяем промпту вслепую: Gemini иногда возвращает не то, что просили
// (дробное число, строку, значение вне 0-100, вообще ничего при __unparsed).
// null — если оценки нет или она совсем не похожа на число, чтобы вызывающий
// код (recognize.js) мог отличить "оценки нет" от "оценка 0" (полностью
// нечитаемый документ — тоже валидный, отличный от отсутствия сценарий).
function normalizeConfidence(value) {
  // Явная проверка null/undefined ДО Number(): Number(null) === 0 (не NaN!),
  // Number(undefined) === NaN — без этой проверки null тихо превращался бы
  // в оценку "0" (полностью нечитаемый документ) вместо "оценки нет".
...
module.exports = { normalizeFields, normalizeItems, normalizeDateValue, normalizeAmountValue, normalizeConfidence };
